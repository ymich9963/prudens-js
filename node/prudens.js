...
const utils = require("./prudensUtils");
    ...
    for (const literal of body) {
        ...
        for (const sub of substitutions) {
            ...
            for (const fact of facts) {
                ...
                // console.log(instance);
                // console.log(fact);
                const unifier = utils.unify(instance, fact, sub);
                // console.log("Unifier:");
                // console.log(unifier);
                // debugger;
                if (unifier !== undefined) {
                    const extension = utils.extend(sub, unifier);
                    // console.log("sub:", sub, "\nextension:", extension);
                    // extended = true;
                    // debugger;
...
function applyToLiteral(sub, literal) {
    ...
    subLiteral["args"] = utils.apply(sub, literal["args"])
    return subLiteral;
}
...
function isMoreSpecific(body1, body2, sub) { // true if body1 is more specific than body2, false otherwise.
	...
	for (const literal2 of body2) {
        ...
        for (const literal1 of body1) {
            unifiable = utils.unify(applyToLiteral(sub, literal1), applyToLiteral(sub, literal2), sub);
...
function jsEvaluation(body, sub, code) { // Checks whether, given a substitution, the corresponding JS predicates hold.
    ...
    for (const literal of body) {
        if (literal["isEquality"]) {
            ...
            // console.log(equality);
            if (equality["unifier"]) {
                sub = utils.extend(sub, equality["unifier"]);
            }
